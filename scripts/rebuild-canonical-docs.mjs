...
export async function rebuildCanonicalDocs(manifestOverride = null, searchIndexOverride = null) {
  ...
  for (const document of manifest) {
    ...
    const existingCanonicalDocument = await readCanonicalDocument(document.slug);
    const autoCanonicalDocument = html
      ? buildCanonicalDocument(document, searchIndexMap.get(document.slug), html)
      : existingCanonicalDocument;

    if (!autoCanonicalDocument) {
      throw new Error(`Не найден источник canonical document для ${document.slug}: отсутствуют viewer.html и content/docs/${document.slug}/document.json.`);
    }
